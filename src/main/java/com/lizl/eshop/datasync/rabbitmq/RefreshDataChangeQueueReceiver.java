...
public class RefreshDataChangeQueueReceiver {
    ...
    Logger logger = Logger.getLogger(RefreshDataChangeQueueReceiver.class);
            ...
            while (true){
                if(! dimDataChangeMessageSet.isEmpty()){
                    for (String messageData : dimDataChangeMessageSet){
                        rabbitMQSender.send("refresh-aggr-data-change-queue", messageData);
                        logger.info("【将去重后的数据变更消息发送到下一个 刷数据 queue】，messageData=" + messageData);
                    }
